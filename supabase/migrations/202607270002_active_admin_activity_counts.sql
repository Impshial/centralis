...
begin
  ...
  assembled as (
    select
      ...
      (select created_at from public.users where id = activity.id and coalesce(deleted, false) = false) as user_account_last,
      (select max(updated_at) from public.element_types where user_id = activity.id and coalesce(deleted, false) = false) as element_types_last,
      (
        select max(ett.updated_at)
        from public.element_type_templates ett
        join public.element_types et on et.id = ett.element_type_id
        where et.user_id = activity.id
          and coalesce(et.deleted, false) = false
          and coalesce(ett.deleted, false) = false
          and coalesce(ett.is_default, false) = false
      ) as templates_last,
