...
begin
  ...
  with mapped_fields as (
    select
      ...
      coalesce(
        ...
        nullif(
          regexp_replace(lower(coalesce(detf.label, detf.id, 'field')), '[^a-z0-9]+', '_', 'g'),
          ''
        ),
        'field'
      ) as field_key,
  ...
  keyed_fields as (
    select
      *,
      case
        when count(*) over (partition by template_id, field_key) = 1 then field_key
        else concat(
          field_key,
          '_',
          regexp_replace(lower(default_field_id), '[^a-z0-9]+', '_', 'g')
        )
